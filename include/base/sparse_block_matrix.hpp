...
#   endif
  ...
  {
    auto dest = FblockSolver::make_unique<SparseBlockMatrix<MatrixTransposedType>>(
  ...
  {
    auto a = FblockSolver::make_unique<SparseBlockMatrix>(&_rowBlockIndices[0], &_colBlockIndices[0], _rowBlockIndices.size(), _colBlockIndices.size());
    add_internal(*a);
    return std::move(a);
  }
